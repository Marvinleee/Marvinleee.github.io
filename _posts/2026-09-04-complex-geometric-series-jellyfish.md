...
\[
a+ar+ar^2+\cdots=\frac{a}{1-r},\qquad |r|<1
\]

但只要把公比 \(r\) 放进复平面，它就活了。令 \(r=\rho e^{i\theta}\)，则每一项

\[
ar^n=|a|\rho^n e^{i(\arg a+n\theta)}
\]
...
> 拖动 **|r|** 滑块从 0.10 一路推到 1.18，你会连续穿过三种数学状态，而不是切换三个动画。按 **H** 键可以隐藏所有 UI，只看画面。
...
\[
h(x,y,t)=\underbrace{s\cdot\tanh\!\left(\frac{|a|}{\sqrt{(1-x)^2+y^2+\epsilon}}\right)}_{\text{真正的 }|f(z)|\text{ 曲面}}
\;+\;
...
\[
ar^n = |a|\rho^n \cdot e^{i(\arg a + n\theta)}
\]

- 模长 \(|a|\rho^n\) 指数衰减 → **向内收缩**；
...
这不是三个视觉主题，而是几何级数自身的三段性质。判据只有 \(|r|\) 与 1 的关系：
...
拖动 **|r|** 这个动作，改变的不是某个动画参数，而是一个数学系统本身的状态。看起来像水母，只是这套动力学恰好长这样。
